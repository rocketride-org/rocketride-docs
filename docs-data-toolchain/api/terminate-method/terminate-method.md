...
- [`use()`](../use-method/use-method.md) - Start a pipeline (returns the token)
- [`get_task_status()` / `getTaskStatus()`](../get-task-status-method/get-task-status-method.md) - Check if the pipeline is still running
- [`send()` / `sendFiles()`](../send-method/send-method.md) - Send data to a pipeline
